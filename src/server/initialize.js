module.exports = function initialize() {
  return function (req, res, next) {
    ...
    if(!req.session.sessionId) {
      ...
      // created, populate req.session.sessionId and req.session.user. 
    ...
    // session expires req.session.user will be left populated with a 
    // non-existent user. 
    // Therefore I need to compare req.session.user against all current 
      ...
      if(err) {
        ...
        db.getAllUsers({}, (err, users) => {
          ...
          if(users && req.session) {
            ...
            if(userMatch === false) {
              // req.session.user does not match one of the existing users. 
